 ...
 * real function.
 */
public class JobsDashboardMain extends ComponentBase {

	private static JobsDashboardMainUiBinder uiBinder = GWT.create(JobsDashboardMainUiBinder.class);
	...
	Label refreshLink;
	@UiField
	FlexTable dashboardPanel;
	@UiField
	FlexTable dashboardPanel1;
	@UiField
	Style style;
	@UiField
	...
	/*
	 * protected void setTimer(final int count) { refreshCounter(count); }
	 */
	 ...
	 * this component.
	 *
	 * @return A widget that the infrastructure should place in the UI; usually a
	 *         panel.
	 */
	@Override
	...
	private void populateTableRow(final String projectName, final String procedureName, final Label lblField,
			final HorizontalPanel hpStatus, final FlexTable ftJobs, final Hidden hRatio, final HTML htmlLastRun) {
		...
		findObjectsRequest.setCallback(new FindObjectsResponseCallback() {
			...
			public void handleResponse(FindObjectsResponse response) {
				...
				if (jobs.isEmpty()) {
					ftJobs.clear();
					// Window.alert("no jobs for "+procedureName+" requested "+m_numberOfjobs+" jobs
					// from project "+projectName);
					ftJobs.setWidget(0, 0, new Label("no jobs found."));
					return;
				}
				ftJobs.clear();
				int currentRowItem = m_numberOfjobs + 3; // number of jobs + first 3 columns for project,procedure,
															// stats
				int successCount = 0;
				...
				// String lastRunDate = "";
				// try{
				// lastRunDate=dt.parse("2009-10-12T00:00.000").toString();
				// 2012-02-22T04:02:39.074Z
				...
				lastJobRunText += "<BR /><B>Elapsed Time:</B> " + (lastJob.getElapsedTime() / 1000) + "sec";
				htmlLastRun.setHTML(lastJobRunText);
				// lblLastRun.setWordWrap(true);
				
				
				int i = dashboardPanel1.getRowCount();
...
//					List<Job> jobs = iResponse.getJobs();
//					Job lastJob = jobs.get(0);
//					JobOutcome currentJobOutcome = lastJob.getOutcome();
	//
//					String jobName = lastJob.getName();
//					String lunchedBy = lastJob.getLaunchedByUser();
					String elapsedTime = ""+(lastJob.getElapsedTime() / 1000);
//					String jobOutcome = currentJobOutcome.toString();

					
					final HTML html0 = new HTML();
					String htmlStr = "";
...
//					html1.setHTML("<table border='1' style='width:100%'><tr><td style='background-color:red'>"+procedureName+"</td></tr></table>");
//					final HTML html2 = new HTML();
//					html2.setHTML("<table border='1' style='width:100%'><tr><td style='background-color:green'>"+"jobOutcome"+"</td></tr></table>");
//					final HTML html3 = new HTML();
//					html3.setHTML("<table border='1' style='width:100%'><tr><td style='background-color:blue'>"+"jobName"+"</td></tr></table>");
					
					dashboardPanel1.setWidget(i, 0, html0);
//					dashboardPanel1.setWidget(row, 1, html1);
//					dashboardPanel1.setWidget(row, 2, html2);
//					dashboardPanel1.setWidget(row, 3, html3);
	...
	public void processDashboardConfig(PropertySheet ps, Label dashboardTitle, Label projectColumnHeader,
			Label procedureColumnHeader) {
	...
	private String createJobUrl(Job currentJob) {
		CommanderUrlBuilder urlBuilder = CommanderUrlBuilder.createLinkUrl("jobDetails", "jobs",
				currentJob.getId().toString());
	...
	private void createConfigureLink() {
		...
		String href = "/commander/pages/ActiveWorkflowDashboard/configureDashboard_run?redirectTo="
				+ CommanderUrlBuilder.createRedirectUrl().buildString();
			...
			public void handleResponse(PropertySheet response) {
				...
				for (Entry<String, Property> entry : response.getProperties().entrySet()) {
					...
					if (prop.isStringProperty()) {
						...
						row++;
						for (Entry<String, Property> projectSheetEntry : prop.getPropertySheet().getProperties()
								.entrySet()) {
							Property currentProp = projectSheetEntry.getValue();
